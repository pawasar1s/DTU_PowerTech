...
saveRes = 1; % over write the files containing results
%solarCap = [0 0.4 0.7 1 1.2 1.5 1.8 2.1 2.4 2.7 3.0 3.3 3.6]; % solar PV capacity multiplier
%solarCap = 2; % solar PV capacity multiplier 
solarCap = 0:0.1:4;
...
if OID_model == 1 
    ...
    store_Gug_V = cell(1, length(solarCap));
    store_Gug_Preal = cell(1, length(solarCap));
    store_Gug_Sreal = cell(1, length(solarCap));
    ...
    store_Gug_Scap = cell(1, length(solarCap)); % max inverter capacity 
    store_Gug_Pcap = cell(1, length(solarCap)); % Pav 
    store_Gug_Pinj = cell(1, length(solarCap)); %Sinj real 
    store_Gug_PF = cell(1, length(solarCap)); %Sinj real
...
elseif OID_model == 2 
    ...
    store_VAR_V = cell(1, length(solarCap));
    store_VAR_Preal = cell(1, length(solarCap));
    store_VAR_Sreal = cell(1, length(solarCap));
    ...
    store_VAR_Scap = cell(1, length(solarCap)); % max inverter capacity 
    store_VAR_Pcap = cell(1, length(solarCap)); % Pav 
    store_VAR_Pinj = cell(1, length(solarCap)); %Sinj real
    store_VAR_PF = cell(1, length(solarCap)); %Sinj real
end
%%
for i = 1 : length(solarCap)
    ...
    %% Guggilam OID model
    if OID_model == 1
        [V, Pcurt, Qc, Vmax, Gug_V, Gug_I2R, Gug_ITot, Gug_Vdrop, Gug_Preal, Gug_Sreal, Gug_PgTot, Gug_QgTot, Gug_I2RTot, Gug_PcTot, Gug_QcTot, Gug_max_Scap, Gug_max_Pcap, Gug_Pinj, Gug_check_PF] = Guggilam(testCase, T, T0, solar, loadHH, multiPer, per, plotting);
        % store results
        ...
        if saveRes == 1
            if Vmax(2) > 1.05
                ...
                noLim_I2R_Drop = table(store_Gug_I2R, store_Gug_I, store_Gug_Scap, store_Gug_Pcap, store_Gug_Pinj, store_Gug_PF);
            ...
            elseif Vmax(2) == 1.05
                ...
                OID_I2R_1pm = table(store_Gug_I2R, store_Gug_I, store_Gug_Preal, store_Gug_Sreal, store_Gug_V, store_Gug_Vdrop, store_Gug_Scap, store_Gug_Pcap, store_Gug_Pinj, store_Gug_PF); % 1x41 cell
    ...
    elseif OID_model == 2
        [V, Pcurt, Qc, Vmax, Gug_V, Gug_I2R, Gug_ITot, Gug_Vdrop, Gug_Preal, Gug_Sreal, Gug_PgTot, Gug_QgTot, Gug_I2RTot, Gug_PcTot, Gug_QcTot, Gug_max_Scap, Gug_max_Pcap, Gug_Pinj, Gug_check_PF] = VoltVar(testCase, T, T0, solar, loadHH, multiPer, per, plotting);    
        % store results
