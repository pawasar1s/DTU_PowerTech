function [V, Pcurt, Qc, Vmax, Gug_V, Gug_I2R, Gug_ITot, Gug_Vdrop, Gug_Preal, Gug_Sreal, Gug_PgTot, Gug_QgTot, Gug_I2RTot, Gug_PcTot, Gug_QcTot, Gug_max_Sinj, Gug_max_Pav, Gug_Pinj, Gug_check_PF] = VoltVar(testCase, T, T0, solar, loadHH, multiPer, per, plotting) 
...
elseif multiPer == 1
    ...
    Gug_PFgTot = complex(zeros(T-T0+1,1)); % PF at the substation/transformer
    ...
    Gug_Preal = complex(zeros(T-T0+1,size(mpc.bus,1)-1)); % PV output injected in the grid
    Gug_Sreal = complex(zeros(T-T0+1,size(mpc.bus,1)-1)); % PV output injected in the grid
    Gug_Pinj = complex(zeros(T-T0+1,size(mpc.bus,1)-1)); % FOR GRAPHS: real inverter capacity considering Pc   
    Gug_Pc = complex(zeros(T-T0+1,size(mpc.bus,1)-1)); 
    Gug_max_Pav = complex(zeros(T-T0+1,size(mpc.bus,1)-1)); 
    Gug_max_Sinj = complex(zeros(T-T0+1,size(mpc.bus,1)-1)); % FOR GRAPHS: inverter capacity based on Pav    
    ...
    for t = T0 : T 
        ...
        Gug_max_Sinj(t-T0+1,:) = Scap; % FOR GRAPHS: inverter capacity based on Pav    
        Gug_max_Pav(t-T0+1,:) = Pcap; 
        % slope with deadband
    ...
    % SAVE RESULTS ===========================================    
        ...
        Pinj = Scap - (Scap - sqrt(Sreal.^2 - Qc.^2));
    ...
    if plotting == 1
        ...
        figure(203)
        plot(Gug_Pinj(:,select_house),'*');hold on; plot(Gug_max_Sinj(:,select_house),'o')
